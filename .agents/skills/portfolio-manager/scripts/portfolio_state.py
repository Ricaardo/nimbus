...
def pull_futu(rates=None):
    ...
    # OpenD 未解锁/掉线路由到模拟盘时，REAL 账户会返回 total_assets=0 且零持仓——
    # 已注资的实盘绝不会真读到"0 资产 + 0 持仓"。视为拉取失败(ok=False)，让调用方
    # 保留上次完整真相文件，避免用残缺快照(仅剩 IBKR)覆盖 → 触发假回撤/假集中度告警。
    if nav_usd == 0 and not positions:
        print("[warn] futu REAL 账户返回 0 资产且无持仓 → 判为拉取失败(OpenD 未解锁?)",
              file=sys.stderr)
        return [], 0.0, 0.0, False
...
def build(_meta=None):
    ...
    # futu 拉取失败(OpenD 掉线/未解锁)时，携带 nav_history 最后一条 futu_usd 保持总 NAV 完整。
    # 否则 NAV 只剩 IBKR(~$2k)、缩水 ~90% → 假回撤告警；且单票 IBKR 占比>100% → 假集中度告警。
    # 与下方 IBKR 现金 carry-forward 同一模式：宁可用陈旧总值，也不让残缺片段刻下假低谷。
    # futu 持仓明细本次缺失(positions 仅剩 IBKR)，恢复 OpenD 后自动更新；携带值不入 nav_history。
    futu_carried = False
    if not fok:
        for row in reversed(load_nav_history()):
            prev_futu = row.get("futu_usd")
            if prev_futu is not None:
                fnav = float(prev_futu)
                futu_carried = True
                break
    ...
    if futu_carried:
        flags.append({"type": "FUTU数据陈旧_携带", "ticker": "FUTU", "severity": "medium",
                      "detail": f"OpenD 未拉到实盘，携带上次 futu NAV ${fnav:,.0f} 保持总盘完整；"
                                "futu 持仓明细本次缺失，恢复 OpenD 后自动更新"})
    if icash_carried:
...
def main():
    ...
    # nav_history 仅在 futu 拉取成功时追加（既有逻辑）：携带值不入历史，避免污染回撤基准。
    if meta.get("futu_ok"):
        append_nav_history(st)

    if args.quiet:
        print(f"已写入 {OUT_FILE}（futu_ok={meta.get('futu_ok')}）：{len(st['positions'])} 持仓，"
