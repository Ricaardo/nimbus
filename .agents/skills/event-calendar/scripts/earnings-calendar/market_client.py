...
Same client surface + the same return shapes the earnings tools expect, but every
value comes from the data-access facade (quote / history / profile /
earnings_calendar) — no direct vendor/data-source access (enforced by
check-tier-boundary.sh §2.5/§4.8/§4.9).
"""
import sys
...
def _short_exchange(long_name: str | None) -> str:
...
class FMPClient:
    ...
    # ── company profile (facade /profile -> legacy FMP keys) ────────────────
        ...
        return [
            {
                ...
                "price": None,  # not in profile; use get_quote for live price
            }
        ]
