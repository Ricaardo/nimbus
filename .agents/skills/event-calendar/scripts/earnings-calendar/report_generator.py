...
def generate_chinese_report(analysis: dict, output_file: str):
    """生成earnings-trade-analyzer 分析中文报告"""
    import json
    
    lines = []
    lines.append("# earnings-trade-analyzer 分析报告")
    lines.append("")
    ...
    lines.append("---")
    lines.append(f"*报告由 earnings-trade-analyzer 自动生成，支持美股/A股/港股多市场*")
    lines.append("")
...
def generate_dashboard_json(analysis: dict, output_file: str):
    ...
    dashboard = {
        "title": "earnings-trade-analyzer 分析 Dashboard",
        "generated_at": datetime.now().isoformat(),
        "charts": [],
        "metrics": metrics[:4] if metrics else [{"label": "状态", "value": "分析完成", "change": "", "trend": "flat"}],
        "summary": "earnings-trade-analyzer 分析分析完成。"
    }
