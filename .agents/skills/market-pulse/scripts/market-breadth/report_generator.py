...
def generate_chinese_report(analysis: dict, output_file: str):
    """生成market-breadth-analyzer 分析中文报告"""
    import json
    
    lines = []
    lines.append("# market-breadth-analyzer 分析报告")
    lines.append("")
    
    metadata = analysis.get("metadata", {})
    lines.append(f"**生成时间:** {metadata.get('generated_at', 'N/A')}")
    lines.append(f"**数据源:** {metadata.get('data_source', 'FMP API')}")
    lines.append("")
    ...
    lines.append("---")
    lines.append(f"*报告由 market-breadth-analyzer 自动生成，支持美股/A股/港股多市场*")
    lines.append("")
    lines.append("**免责声明:** 本分析仅供参考，不构成投资建议。")
...
def generate_dashboard_json(analysis: dict, output_file: str):
    ...
    dashboard = {
        "title": "market-breadth-analyzer 分析 Dashboard",
        "generated_at": datetime.now().isoformat(),
        "charts": [],
        "metrics": metrics[:4] if metrics else [{"label": "状态", "value": "分析完成", "change": "", "trend": "flat"}],
        "summary": "market-breadth-analyzer 分析分析完成。"
    }
