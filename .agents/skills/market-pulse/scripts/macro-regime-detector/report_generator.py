...
def generate_chinese_markdown_report(analysis: dict, output_file: str):
    """生成中文宏观体制检测报告"""
    lines = []
    composite = analysis.get("composite", {})
    regime = analysis.get("regime", {})
    ...
    # 报告头部
    lines.append("# 宏观体制检测报告")
    lines.append("")
    lines.append(f"**生成时间:** {metadata.get('generated_at', 'N/A')}")
    lines.append(f"**数据源:** FMP API ({metadata.get('api_calls', {}).get('api_calls_made', 'N/A')} 次调用)")
    lines.append("")

    # 第一部分：当前体制评估
    lines.append("---")
    lines.append("")
    lines.append("## 1. 当前体制评估")
    lines.append("")
    ...
    regime_zh = {
        "Concentration": "集中化（Concentration）",
        "Broadening": "扩散化（Broadening）",
        "Contraction": "收缩（Contraction）",
        "Inflationary": "通胀（Inflationary）",
        "Transitional": "过渡期（Transitional）",
    }
    regime_label_zh = regime_zh.get(regime_label, regime_label)
    confidence_zh = {"high": "高", "medium": "中", "low": "低"}.get(confidence.lower(), confidence)

    lines.append("| 指标 | 值 |")
    lines.append("|------|-----|")
    lines.append(f"| **当前体制** | **{regime_label_zh}** |")
    lines.append(f"| **转换置信度** | {confidence_zh} |")
    lines.append(f"| **转换评分** | **{score}/100** |")
    lines.append(f"| **信号区间** | {composite.get('zone', 'N/A')} |")
    lines.append(f"| **转换概率** | {transition.get('probability_range', 'N/A')} ({transition.get('level', 'N/A')}) |")
    lines.append(f"| **信号组件数** | {composite.get('signaling_components', 0)}/6 |")
    lines.append("")

    lines.append(f"> **体制说明:** {regime.get('regime_description', '')}")
    lines.append("")
    lines.append(f"> **投资指引:** {composite.get('guidance', '')}")
    lines.append("")

    # 第二部分：组件详情
    lines.append("---")
    lines.append("")
    lines.append("## 2. 六大组件信号")
    lines.append("")

    component_names_zh = {
        "concentration": "市场集中度（RSP/SPY）",
        "yield_curve": "收益率曲线（10Y-2Y）",
        "credit_conditions": "信用环境（HYG/LQD）",
        "size_factor": "规模因子（IWM/SPY）",
        "equity_bond": "股债关系（SPY/TLT）",
        "sector_rotation": "行业轮动（XLY/XLP）",
    }

    lines.append("| 组件 | 评分 | 信号 |")
    lines.append("|------|------|------|")
    for key, name_zh in component_names_zh.items():
        comp = components.get(key, {})
        lines.append(f"| {name_zh} | {comp.get('score', 'N/A')} | {comp.get('signal', 'N/A')[:80]} |")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"- **当前体制**: {regime_label_zh}")
    lines.append(f"- **建议操作**: {composite.get('guidance', '维持当前配置')}")
    lines.append(f"- **推荐仓位**: {regime.get('portfolio_posture', 'N/A')}")
    lines.append("")
    ...
    lines.append("---")
    lines.append("*报告由 macro-regime-detector 自动生成*")
    lines.append("")
    lines.append("**免责声明:** 本分析仅供教育和参考目的，不构成投资建议。体制检测具有固有的不确定性，信号可能产生误报。过去的体制模式不一定能预测未来的转换。请自行研究并在做出投资决策前咨询专业财务顾问。")

    with open(output_file, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"中文报告已保存至: {output_file}")


def generate_dashboard_json(analysis: dict, output_file: str):
    """生成 Recharts 兼容的 Dashboard JSON"""
    import json
    ...
    for key, name_zh in component_names_zh.items():
        comp = components.get(key, {})
        bar_data.append({"name": name_zh, "score": comp.get("score", 0)})

    dashboard = {
        "title": "宏观体制检测 Dashboard",
        ...
        "metrics": [
            {"label": "综合评分", "value": composite.get("composite_score", 0), "change": "", "trend": "flat"},
            {"label": "当前体制", "value": regime.get("regime_label", "Unknown"), "change": "", "trend": "flat"},
            {"label": "转换概率", "value": regime.get("transition_probability", {}).get("probability_range", "N/A"), "change": "", "trend": "flat"},
            {"label": "信号组件数", "value": f"{composite.get('signaling_components', 0)}/6", "change": "", "trend": "flat"},
        ],
        "summary": f"当前宏观体制为{regime.get('regime_label', 'Unknown')}，综合评分{composite.get('composite_score', 0)}/100，{composite.get('signaling_components', 0)}个组件发出信号。",
    }

    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(dashboard, f, ensure_ascii=False, indent=2)
    print(f"Dashboard JSON 已保存至: {output_file}")
