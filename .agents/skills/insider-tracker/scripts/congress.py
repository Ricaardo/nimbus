#!/usr/bin/env python3
"""国会交易 — QuiverQuant 免费 live 端点(无需 key,披露滞后数日)。
最新 ~1000 条美众议院议员交易披露。按议员/标的筛选;成功写本地快照,失败回退。
用法:
  congress.py                  # 最新值得注意
...
def fetch():
    ...
    if os.path.exists(SNAP):
        with open(SNAP) as f:
            return json.load(f).get("data", []), True
...
def main():
    ...
    data, stale = fetch()
    if a.rep:
        data = [x for x in data if a.rep.lower() in str(x.get("Representative", "")).lower()]
    if a.ticker:
        data = [x for x in data if a.ticker.upper() == str(x.get("Ticker", "")).upper()]
    rows = sorted(data, key=lambda x: x.get("ReportDate", ""), reverse=True)[: a.top]
    ...
    print(f"{title}{note}\n")
    if not rows:
        print("（无匹配；该端点为最新 ~1000 条窗口，此人/标的近期可能无披露）")
        return
    for x in rows:
