...
### 看日志 / 成本 / 记忆
```bash
tail -f ~/nimbus-os/nimbus/logs/nimbus.stderr.log
# 每日每模型成本:
sqlite3 ~/nimbus-os/nimbus/data/state.db "select date(ts/1000,'unixepoch','localtime') d,model,round(sum(cost_usd),3) cost,count(*) n from usage group by 1,2 order by 1 desc"
# 学到的偏好/教训:
sqlite3 ~/nimbus-os/nimbus/data/state.db "select kind,text from memories where active=1"
# 决策台账:
sqlite3 ~/nimbus-os/nimbus/data/state.db "select symbol,direction,status,rationale from decisions order by id desc limit 20"
```

### 改代码后重载(daemon 自动拉新代码)
```bash
cd ~/nimbus-os/nimbus && bun test && bun run typecheck   # 先确认绿
...
**Token/密钥**(勿删):`~/.claude/channels/discord/.env`、`~/nimbus-os/nimbus/secrets/mcp.json`、长桥 OAuth(Claude Code 凭证库)。
