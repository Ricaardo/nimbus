...
>
> ⚠️ **历史规划档（部分技能后续已变更，2026-07 精简）**：`serenity-tracker` 已升级为 `hot-tickers`（网络热门标的聚合，followserenity 并入一源）；`congress-tracker`、`analyst-ratings` 因零使用已删除。下文 §3 的相关「✅已建」条目按此理解。
...
| Serenity(白毛股神) | ❌ 无 | ✅ followserenity HTML `<tr>` 表格可解析(532 thesis) | **✅ 已建 serenity-tracker** §3.1 |
...
- **落地**：移植 btcdca 小程序 `social-feed.js` 的 `extractSerenityRows` 解析器到 Python（`<tr>`→ticker/公司/收益%/conviction/thesis/x.com 源），快照兜底。
- **实测**：SIVE +890% / AXTI +626% / PL +196% / NVDA +17.81% 等，含瓶颈逻辑。
- **触发**：「白毛股神/Serenity/瓶颈理论/aleabitoreddit」；`--ticker` 筛。⚠️ 公开帖抽取非真实13F、未审计、NFA。
