...
const morningReport: Module = {
  ...
  async handle(ctx: ModuleContext): Promise<void> {
    const prompt = [
      ...
      '',
      '1. 调用 `market-pulse` skill（MHS 0-100评分模式）评估当前美股市场温度，给出 MHS 分数和仓位指引；并一句话给当前 regime（牛/熊/震荡）判断。',
      '2. 调用 `event-calendar` skill 列出今日及未来3天的关键财报/宏观事件，标注对主人持仓的影响等级（持仓标的财报当天要特别醒目）。',
      '3. 调用 `thesis-tracker` skill 做 decay 检查：有无超过30天未更新的论点？有无价格已大幅偏离的持仓？',
      '4. 调用 `portfolio-manager` skill（拉 futu+IBKR 真实持仓）检查偏离：当前集中度 vs 目标，有无再平衡信号；给组合层面昨夜整体涨跌与最大回撤标的。',
      '5. 对主要持仓用 `futu-*-anomaly`（资金/技术异动）skill 扫一遍：有无主力资金异动、技术形态异动、量价背离等需要今天盯的信号？只报真正异常的，无异常就说"无异动"。',
      '',
