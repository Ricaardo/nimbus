...
- 优先 **OpenBB**（python CLI,零 MCP 成本）:
  ```bash
  ~/nimbus-stack/nimbus/.venv-openbb/bin/python -c "from openbb import obb; print(obb.equity.fundamental.transcript('NVDA', year=2026, provider='fmp').to_df())"
  ```
  （交互式想用 OpenBB MCP 工具,该 run 显式 `mcpAllow: ['openbb']`；默认休眠。）
  **OpenBB 命令速查**：`~/nimbus-stack/nimbus/skills/references/openbb-commands.md`。
...
```bash
cat <<'DOC' | bun run ~/nimbus-stack/nimbus/scripts/kb-ingest.ts --kind earnings_call --ticker NVDA --title "NVDA FY26Q1 电话会"
<四维分析正文,含与上季对照>
DOC
```
