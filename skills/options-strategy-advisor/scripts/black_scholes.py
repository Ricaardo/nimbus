...
def fetch_historical_prices_for_hv(symbol, api_key=None, days=90):
    """
    Fetch historical prices for HV calculation (multi-market support)

    Parameters:
    -----------
    symbol : str
        Stock ticker (supports US, A-shares, HK, Crypto, Commodities)
    api_key : str, optional
        FMP API key (only needed for US stocks via FMP)
    days : int
    ...
    try:
        ...
        client = DataClient(fmp_api_key=api_key)
        df = client.get_historical(symbol, days=days)
        if not df.empty:
...
def get_current_stock_price(symbol, api_key=None):
    """Fetch current stock price (multi-market support)"""
    # 尝试 DataClient
    try:
        import os, sys
        shared_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'shared')
