import { WebSocketTransport, SubscriptionClient, InfoClient, HttpTransport, } from "@nktkas/hyperliquid";
...
export class SubscriptionManager {
    ...
    cache;
    subscriptions = [];
    perpMetaInterval = null;
    spotMetaInterval = null;
    isTestnet;
    log;
    constructor(cache, isTestnet, log) {
        this.cache = cache;
    ...
    async start() {
        ...
        const midsSub = await this.subscriptionClient.allMids({ dex: "ALL_DEXS" }, (event) => {
            this.cache.setAllMids(event.mids);
        });
