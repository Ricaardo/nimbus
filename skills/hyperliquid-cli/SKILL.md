...
description: "Hyperliquid DEX CLI for perpetual futures trading. Check prices, order books, positions, balances. Place limit/market/stop orders, cancel orders, adjust leverage. Requires `hl` CLI in PATH. Triggers on 'hyperliquid', 'hl', 'perp futures'."
---
...
> ⚠️ 风控联动：永续合约带杠杆，下任何交易单前先走 `trade-execution`（pre-mortem + 仓位/止损），交易后用 `trade-journal` 记录、用 `thesis-tracker` 跟踪论点。
