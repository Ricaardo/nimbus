...
- **Binance Skills Hub**: For CEX spot trading when Hyperliquid doesn't list a token
- **Hummingbot Arbitrage**: Check Hyperliquid funding rates vs spot prices for funding rate arbitrage opportunities
- **CoinMarketCap 官方 MCP (`cmc-mcp`)**: Cross-reference free/basic market data, Fear & Greed, dominance, and global metrics before opening large positions. Do not use CMC x402, private-key, pay-per-request, or paid historical endpoints from this skill.
- **Almanak SDK**: For automated DeFi strategy execution with backtesting (complements manual Hyperliquid trading)
