...
# Extended ETFs for global/China-focused analysis
CHINA_ETFS = ["FXI", "KWEB", "MCHI", "GLD", "SLV"]
CRYPTO_ETFS = ["BITO"]
HISTORY_DAYS = 600  # ~2.4 years of daily data

# CN 市场 ETF 映射（用 A 股指数/ETF 替代美股 ETF）
CN_ETF_MAP = {
    "SPY": "000300.SH",    # 沪深300 ← S&P 500
    "RSP": "000905.SH",    # 中证500 ← 等权重 S&P (代理广度)
    "IWM": "000852.SH",    # 中证1000 ← Russell 2000
    "TLT": "511260.SH",    # 国债 ETF ← 长期国债
    "SHY": "511010.SH",    # 短期国债 ETF ← 短期国债
    "HYG": "511260.SH",    # 国债 ETF (A 股无高收益债 ETF)
    "LQD": "511260.SH",    # 国债 ETF (A 股无投资级债 ETF)
    "XLY": "159928.SZ",    # 消费 ETF ← 非必需消费
    "XLP": "159928.SZ",    # 消费 ETF (A 股消费不分必需/非必需)
}
...
def parse_arguments():
    ...
    parser.add_argument(
        "--market",
        choices=["us", "cn", "global"],
        default="us",
        help="Market focus: us (default), cn (China-focused), global",
    )
    parser.add_argument(
        "--lang",
        choices=["en", "zh"],
...
def main():
    ...
    # Initialize data client based on market
    use_akshare = (args.market == "cn")
    akshare_client = None
    client = None
    ...
    if use_akshare:
        ...
        try:
            ...
            print("AKShare 客户端已初始化（CN 市场模式）")
        except Exception as e:
            print(f"WARNING: AKShare 初始化失败 ({e})，回退 FMP", file=sys.stderr)
            use_akshare = False
    ...
    # Build ETF list based on market focus
    etf_list = list(REQUIRED_ETFS)
    if args.market in ("cn", "global"):
        etf_list.extend(CHINA_ETFS)
    if args.market == "global":
        etf_list.extend(CRYPTO_ETFS)

    # ================================================================
    # Step 1: Fetch Market Data
    # ================================================================
    print()
    print(f"Step 1/4: Fetching Market Data ({len(etf_list)} ETFs)")
    print("-" * 70)

    historical = {}

    if use_akshare:
        # CN 市场：通过 AKShare 获取 A 股指数数据
        for etf in etf_list:
            cn_ticker = CN_ETF_MAP.get(etf, etf)
            print(f"  Fetching {etf} → {cn_ticker} ({args.days} days)...", end=" ", flush=True)
            try:
                df = akshare_client.get_stock_price(cn_ticker, count=args.days)
                if not df.empty:
                    # 转换为 FMP 兼容格式 (list of dicts, 最新在前)
                    records = df.to_dict("records")
                    for r in records:
                        if "date" in r:
                            r["date"] = str(r["date"])[:10]
                    historical[etf] = list(reversed(records))
    ...
    treasury_rates = None
    if use_akshare:
        print("  CN 市场模式: 跳过 Treasury rates（使用 SHY/TLT fallback）")
    else:
    ...
    analysis = {
        "metadata": {
            ...
            "data_source": "AKShare" if use_akshare else "FMP API",
            "market": args.market,
            "history_days": args.days,
            "api_calls": client.get_api_stats() if client else {"source": "akshare"},
            "etfs_analyzed": REQUIRED_ETFS,
            "cn_etf_mapping": CN_ETF_MAP if use_akshare else None,
    ...
    # Generate Chinese report and dashboard
    if args.lang == "zh":
        zh_file = os.path.join(args.output_dir, f"macro_regime_{timestamp}_zh.md")
        generate_chinese_markdown_report(analysis, zh_file)

    dashboard_file = os.path.join(args.output_dir, f"macro_regime_dashboard_{timestamp}.json")
    generate_dashboard_json(analysis, dashboard_file)
    ...
    if client:
        ...
        print("Data Source: AKShare (CN 市场)")
    print()
