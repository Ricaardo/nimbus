...
# CN 市场指数映射
CN_INDEX_MAP = {
    "^GSPC": "000300.SH",   # 沪深300 ← S&P 500
    "QQQ": "399006.SZ",     # 创业板指 ← NASDAQ
    "^VIX": None,           # A 股无 VIX（跳过）
}
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="us",
        help="Market focus: us (default), cn (A-shares), global",
    )
...
def main():
    ...
    # Optional: Crypto market signal (BTC price/volatility)
    crypto_signal = None
    if args.market == "global":
        try:
            ...
            crypto = CryptoClient()
            btc_data = crypto.get_24hr("BTC")
            if btc_data:
                crypto_signal = {
                    "btc_price": btc_data.get("price"),
                    "btc_24h_change_pct": btc_data.get("price_change_pct"),
                    "source": btc_data.get("source"),
                }
                print(f"  [Crypto] BTC: ${btc_data['price']:.0f} ({btc_data.get('price_change_pct', 0):+.1f}%)")
        except Exception as e:
            print(f"  [Crypto] BTC signal unavailable: {e}")
